...
const PaymentsPage = () => {
  ...
  const StatusBadge = ({ status }) => {
    return (
      <span className={`px-3 py-1 rounded-full border text-xs font-medium flex items-center justify-center w-fit gap-1 ${getStatusColor(status)}`}>
        {status === 'Success' && <span className="w-1.5 h-1.5 rounded-full bg-green-500" />}
        {status === 'Pending' && <span className="w-1.5 h-1.5 rounded-full bg-orange-500" />}
  ...
  return (
      ...
      <div className="grid grid-cols-4 gap-6 mb-8">
        <Card icon={<FaDollarSign className="text-green-600" />} iconBg="bg-green-100" title="Total Revenue" value="$2,410.83" />
        <Card icon={<FaCheckCircle className="text-blue-600" />} iconBg="bg-blue-100" title="Successful Payments" value="8" />
        <Card icon={<FaClock className="text-yellow-600" />} iconBg="bg-yellow-100" title="Pending Payments" value="3" />
        <Card icon={<FaTimesCircle className="text-red-600" />} iconBg="bg-red-100" title="Failed Payments" value="2" />
      </div>
      ...
      <div className="flex gap-6 mb-8">
        <div className="flex items-center bg-white border border-gray-200 rounded-xl px-4 py-3 w-full shadow-sm">
          ...
          <input
            type="text"
            placeholder="Search by customer, company, payment ID, or transaction ID..."
            value={searchTerm}
      ...
      <div className="grid grid-cols-4 gap-6 mb-8">
        <Card icon={<FaChartLine className="text-purple-600" />} iconBg="bg-purple-100" title="Monthly Growth" value="+18.5%" sub="Compared to last month" />
        <Card icon={<FaUsers className="text-blue-600" />} iconBg="bg-blue-100" title="Active Customers" value="1,247" sub="Currently subscribed" />
        <Card icon={<FaDollarSign className="text-cyan-600" />} iconBg="bg-cyan-100" title="Average Transaction" value="$301.35" sub="Per successful payment" />
        <Card icon={<FaCalendarAlt className="text-green-600" />} iconBg="bg-green-100" title="This Month" value="$2,410.83" sub="February 2026" />
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm flex justify-between mb-8">
        <PaymentMethod icon={<FaCreditCard className="text-blue-600" />} iconBg="bg-blue-100" name="Credit Card" count="5" percent="38% of total" />
        <PaymentMethod icon={<FaDollarSign className="text-green-600" />} iconBg="bg-green-100" name="Bank Transfer" count="2" percent="15% of total" />
        <PaymentMethod icon={<FaCreditCard className="text-purple-600" />} iconBg="bg-purple-100" name="PayPal" count="1" percent="8% of total" />
      </div>
      ...
      <div className="grid grid-cols-4 gap-6 mb-8">
        <Card icon={<FaSync className="text-indigo-600" />} iconBg="bg-indigo-100" title="Total EMI Plans" value="5" />
        <Card icon={<FaCheckCircle className="text-green-600" />} iconBg="bg-green-100" title="Active EMIs" value="4" />
        <Card icon={<FaTimesCircle className="text-red-600" />} iconBg="bg-red-100" title="Overdue EMIs" value="1" />
        <Card icon={<FaCalendarAlt className="text-blue-600" />} iconBg="bg-blue-100" title="EMI Revenue" value="$475.83" />
      </div>
          ...
          <table className="min-w-full text-sm text-left">
            <thead className="bg-gray-50 text-gray-500">
              <tr>
                ...
                <th className="px-4 py-3 font-medium">Paid/Remaining</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {[
                ...
                <tr key={index} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-4 text-gray-700">{item.plan}</td>
                  ...
                  <td className="px-4 py-4 text-gray-700">{item.tenure}</td>

                  <td className="px-4 py-4 text-gray-700">{item.paid}</td>

                  <td className="px-4 py-4 text-gray-700">{item.nextDue}</td>
                  ...
                  <td className="px-4 py-4">
                    <button 
                      onClick={() => handleViewTransaction(item.name, item.status)}
            ...
            <tbody className="divide-y divide-gray-100">
              {transactions.map((row) => (
                <tr key={row.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-sm font-bold text-gray-800">{row.customer}</td>
                  <td className="px-6 py-4 text-sm text-gray-400">{row.company}</td>
                  <td className="px-6 py-4 text-sm text-gray-500">{row.plan}</td>
                  <td className="px-6 py-4 text-sm font-bold text-gray-900">{row.amount}</td>
                  <td className="px-6 py-4 text-sm text-gray-500">{row.method}</td>
                  <td className="px-6 py-4">
                    <StatusBadge status={row.status} />
                  </td>
                  <td className="px-6 py-4 text-center">
                    <button 
                      onClick={() => handleViewTransaction(row.id, row.status)}
