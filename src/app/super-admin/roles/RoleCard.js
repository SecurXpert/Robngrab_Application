...
import { FaEdit, FaTrash, FaUsers, FaArrowRight, FaShieldAlt } from 'react-icons/fa';

export default function RoleCard({ role, onViewPermissions }) {
  return (
    <div className="bg-white rounded-lg border border-gray-200 p-4 sm:p-6 hover:shadow-lg transition-all duration-200 shadow-sm w-full max-w-full overflow-hidden">
      {/* Header with shield icon and action buttons */}
      <div className="flex items-start justify-between mb-4">
        <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-lg border-2 ${role.iconColor} bg-white flex items-center justify-center shadow-sm flex-shrink-0`}>
          <FaShieldAlt className="w-5 h-5 sm:w-6 sm:h-6 text-gray-700" />
        </div>
        <div className="flex gap-2 flex-shrink-0">
          {!role.protected && (
            <>
              <button className="p-2 rounded-lg hover:bg-gray-100 transition-colors group">
                <FaEdit className="w-4 h-4 text-gray-500 group-hover:text-gray-700" />
              </button>
              <button className="p-2 rounded-lg hover:bg-gray-100 transition-colors group">
                <FaTrash className="w-4 h-4 text-gray-500 group-hover:text-red-500" />
              </button>
            </>
          )}
        </div>
