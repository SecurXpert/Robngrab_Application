...
export default function HiringMetricsDashboard({ onModalOpen }) {
  return (
    <div className="mb-6">
        ...
        {Object.entries(CARD_CONFIG).map(([key, config]) => (
          <div key={key} className="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
            <div className="flex justify-between items-center mb-4">
              ...
              <a 
                onClick={() => key === 'shortlisted' ? onModalOpen('shortlisted') : key === 'interviewProcess' ? onModalOpen('interviewProcess') : onModalOpen(config.modal)}
