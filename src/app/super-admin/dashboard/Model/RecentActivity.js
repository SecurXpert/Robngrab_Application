...
export default function RecentActivity() {
  ...
  return (
    <div className="bg-white h-full overflow-y-auto rounded-xl shadow-sm p-6 border border-gray-200 mb-5 text-left">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-family-inter font-weight-500 text-[#0A0A0A]">Recent Activity</h3>
        ...
        {activities.map((activity, index) => (
          <div key={index} className="flex items-center justify-between py-2 border-b border-gray-100 last:border-0">
            <div className="flex-1 space-y-2">
              <div className="text-sm font-medium text-[#0A0A0A]">{activity.action}</div>
