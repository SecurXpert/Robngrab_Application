...
import { 
  ...
  MODAL_CONFIG, 
  CARD_CONFIG 
} from '@/utils/vendorConstants';

export const useVendorData = (vendorSlug) => {
  // State management
    ...
    if (typeof window !== 'undefined') {
      ...
      if (savedVendors) {
        ...
        vendors = vendors.map(vendor => ({
          ...vendor,
          name: vendor.name === 'TechCorp Solutions' ? 'Tech Solutions Inc.' : vendor.name,
          description: vendor.description || 'Detailed analytics and performance insights',
          location: (vendor.name === 'TechCorp Solutions' || vendor.name === 'Tech Solutions Inc.') ? 'Silicon Valley, CA, USA' : (vendor.location || 'Not specified'),
  ...
  // Find the vendor by ID or slug
  const vendor = vendors.find(v => 
    v.id?.toString() === vendorSlug || 
    v.slug === vendorSlug || 
    (v.name && v.name.toLowerCase().replace(/[^a-z0-9]+/g, '-') === vendorSlug)
  ) || vendors[0];
  ...
  const handleUpdateVendor = (updatedVendorData) => {
    ...
    if (savedVendors) {
      ...
      const updatedVendors = vendors.map(v => 
        (v.id?.toString() === vendorSlug || v.slug === vendorSlug || (v.name && v.name.toLowerCase().replace(/[^a-z0-9]+/g, '-') === vendorSlug))
          ? { ...v, ...updatedVendorData }
          : v
      );
  ...
  return {
    ...
    modalActivities: MODAL_ACTIVITIES,
    metrics,
